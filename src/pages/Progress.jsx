            ...
            <div className="flex gap-1.5 mb-2 overflow-x-auto no-scrollbar">
              {METRICS.map(m => (
                <button key={m.key} onClick={() => setMetric(m.key)}
                  className={`chip shrink-0 ${metric === m.key ? 'bg-accent text-white border-accent' : ''}`}>{m.label}</button>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-1.5 justify-center">
            {RANGES.map(r => (
              <button key={r.label} onClick={() => setRange(r.days)}
                className={`chip ${range === r.days ? 'bg-accent text-white border-accent' : ''}`}>{r.label}</button>
            ))}
          </div>
        </>
      )}
